...
<div><h3 class='inline'><{$smarty.const._MD_MYLINKS_CATEGORY}></h3>
    <div class='right'>
        <a href="javascript:mltoggleEffect('<{$mylinks_imgurl}>/icons', 'resizemlcatblock', 'mlsizecatimage')"><img id='mlsizecatimage' src='<{$mylinks_imgurl}>/icons/minimize.gif'
                                                                                                                    alt='<{$lang_minimizeblock}>' title='<{$lang_minimizeblock}>'/></a>
    </div>
</div>
<hr/>
<div id='resizemlcatblock' class='block'>
    <table class='center bnone marg5' style='padding: 0;'>
        <tr>
            <!-- Start category loop -->
            ...
            <td class='aligntop'>
                <{if $category.image != ''}>
                <a href='<{$mylinks_weburl}>/viewcat.php?cid=<{$category.id}>'><img src='<{$category.image}>' style='height: 50px; border-width: 0;' alt=''/></a>
                <{/if}>
            </td>
            <td class='aligntop width30'><a href='<{$mylinks_weburl}>/viewcat.php?cid=<{$category.id}>'><strong><{$category.title}></strong></a>&nbsp;(<{$category.totallink}>)<br><{$category.subcategories}>
            </td>
    ...
    <tr>
        <td class='left'>
            ...
            <a href='<{$mylinks_weburl}>/atom.php' target='_blank'>
                <img src='<{$mylinks_imgurl}>/icons/atom.png' alt='<{$lang_atomfeed}>' title='<{$lang_atomfeed}>'/></a>
