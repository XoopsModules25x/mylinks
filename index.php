...
//$categoryHandler = xoops_getModuleHandler('category', $xoopsModule->getVar('dirname'));
$criteria = new \CriteriaCompo();
$criteria->add(new \Criteria('pid', 0, '='));
$criteria->setSort('title');
$catObjs = $categoryHandler->getAll($criteria);
...
foreach ($catObjs as $catObj) {
    ...
    $childCount = (int)$categoryHandler->getCount($criteria);

    //now retrieve the info for the first 'subcatLimit' categories
    $criteria->setLimit($subcatLimit);
    $criteria->setSort('title');
    $childFields     = ['pid', 'title'];
    $childTitleArray = $categoryHandler->getAll($criteria, $childFields, false, false);
    ...
    $totalLink = Utility::getTotalItems($catObj->getVar('cid'), 0, '>');
    $xoopsTpl->append(
        'categories',
        [
            'image'         => $catImgUrl,
...
$result          = $xoopsDB->query(
    'SELECT l.lid, l.cid, l.title, l.url, l.logourl, l.status, l.date, l.hits, l.rating, l.votes, l.comments, t.description FROM ' . $xoopsDB->prefix('mylinks_links') . ' l, ' . $xoopsDB->prefix('mylinks_text') . ' t WHERE l.status>0 AND l.lid=t.lid ORDER BY date DESC',
    $helper->getConfig('newlinks'),
    0
);
while (list($lid, $cid, $ltitle, $url, $logourl, $status, $time, $hits, $rating, $votes, $comments, $description) = $xoopsDB->fetchRow($result)) {
    if ($xoopsUser && $xoopsUser->isAdmin($xoopsModule->mid())) {
        $isadmin   = true;
        $adminlink = "<a href='" . XOOPSMYLINKURL . "/admin/main.php?op=modLink&amp;&fct=mylinks&amp;lid={$lid}'><img src='" . Utility::getIconURL('edit.png') . "' style='border-width: 0px;' alt='" . _MD_MYLINKS_EDITTHISLINK . "'></a>";
    } else {
        $isadmin   = false;
        $adminlink = '';
    }
